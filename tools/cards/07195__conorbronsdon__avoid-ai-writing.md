...
- **分类**：画龙补充 / 扩容入库 — 补充源
- **链接**：https://github.com/conorbronsdon/avoid-ai-writing
- **Stars**：2595
- **语言**：JavaScript
- **License**：MIT
- **Topics**：ai-writing, claude, claude-code, llm, prompt-engineering, skill, writing
- **GitHub 描述**：Skill that audits and rewrites content to remove AI writing patterns. Use it with your favorite agents including Claude Code, OpenClaw, and Hermes.
- **本地描述**：avoid-ai-writing
- **拉取时间**：2026-07-25 19:13:46
...
See [`cursor-rules/README.md`](https://github.com/conorbronsdon/avoid-ai-writing/blob/main/cursor-rules/README.md) for activation globs and trigger phrases. Functionally identical to the Claude Code skill — same tier vocabulary, same context profiles, same modes.
...
> Representative examples from the catalog — not the exhaustive list (that's [`SKILL.md`](https://github.com/conorbronsdon/avoid-ai-writing/blob/main/SKILL.md)). The skill's human-facing prose catalog and the [detector engine](https://github.com/conorbronsdon/avoid-ai-writing/tree/main/detector/) use **different counts on purpose**: the engine implements 45 `type` categories because it splits the vocabulary tiers and adds stylometric/fingerprint signals (punctuation distribution, function-word entropy, bypass-trick detection) that work as math over a document rather than as a rule you'd look up. The two are mapped in [`detector/CATEGORIES.md`](https://github.com/conorbronsdon/avoid-ai-writing/blob/main/detector/CATEGORIES.md); don't "fix" one count to match the other.
...
[`detector/`](https://github.com/conorbronsdon/avoid-ai-writing/tree/main/detector/) — the same 45-category engine the rules above
...
See [`detector/README.md`](https://github.com/conorbronsdon/avoid-ai-writing/blob/main/detector/README.md) for the full `analyzeText` API
and [`detector/CATEGORIES.md`](https://github.com/conorbronsdon/avoid-ai-writing/blob/main/detector/CATEGORIES.md) for the rule ↔ category
