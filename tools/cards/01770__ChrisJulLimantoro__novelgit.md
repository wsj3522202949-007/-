...
- **Optional** — **Gemini Embedding** when `embeddingProvider` is set to `"gemini"` in Admin; reindex uses [`lib/ai/embeddings-gemini.ts`](https://github.com/ChrisJulLimantoro/novelgit/blob/main/lib/ai/embeddings-gemini.ts) and `GEMINI_API_KEY`. A **full reindex** is required after switching providers.
...
3. Top chunks are returned (see [`lib/ai/manuscript-chat-retrieval.ts`](https://github.com/ChrisJulLimantoro/novelgit/blob/main/lib/ai/manuscript-chat-retrieval.ts))
...
On **reindex**, each chapter can be **distilled** (summary, entities, tags) via **Gemini** ([`lib/ai/chapter-distillation.ts`](https://github.com/ChrisJulLimantoro/novelgit/blob/main/lib/ai/chapter-distillation.ts)). Summaries feed **Global Bible** generation or incremental patching ([`lib/ai/global-bible.ts`](https://github.com/ChrisJulLimantoro/novelgit/blob/main/lib/ai/global-bible.ts)). Model IDs come from `ai-config.json` (`distillationModel`, `bibleRebuildModel`, `biblePatchModel`). If `GEMINI_API_KEY` is unset, distillation and Global Bible generation are skipped.
...
- **Runtime settings** — `/admin` reads and writes **`ai-config.json`** at the root of the **GitHub content repo** (not this app repo). Schema: [`types/ai-config.ts`](https://github.com/ChrisJulLimantoro/novelgit/blob/main/types/ai-config.ts) (`distillationModel`, `bibleRebuildModel`, `biblePatchModel`, `embeddingProvider`, `geminiEmbeddingModel`).
...
- **Gemini (Google AI)** — REST for distillation, Global Bible, and optional embeddings ([`lib/ai/gemini.ts`](https://github.com/ChrisJulLimantoro/novelgit/blob/main/lib/ai/gemini.ts), [`lib/ai/embeddings-gemini.ts`](https://github.com/ChrisJulLimantoro/novelgit/blob/main/lib/ai/embeddings-gemini.ts))
...
| Doc | Contents |
|-----|----------|
| [docs/design-docs.md](https://github.com/ChrisJulLimantoro/novelgit/blob/main/docs/design-docs.md) | Architecture, content schema, data flow |
| [docs/spec/ai-rag-plan.md](https://github.com/ChrisJulLimantoro/novelgit/blob/main/docs/spec/ai-rag-plan.md) | Lore + RAG + AI routes (original spec) |
| [docs/nextjs-coding-guide.md](https://github.com/ChrisJulLimantoro/novelgit/blob/main/docs/nextjs-coding-guide.md) | Next.js 16 / React 19 conventions |
| [docs/ui-ux.md](https://github.com/ChrisJulLimantoro/novelgit/blob/main/docs/ui-ux.md) | UI stack and component patterns |
...
This project is licensed under the [MIT License](https://github.com/ChrisJulLimantoro/novelgit/blob/main/LICENSE).
