...
Drag to rotate. This is not an image or a video: it is an interactive 3D model rendered natively by GitHub. The avatar was generated from a one-line text prompt on [three.ws Forge](https://three.ws/forge) (free tier), decimated to 1,200 triangles, and embedded as ASCII STL with [readme-3d](https://github.com/nirholas/three.ws/blob/main/packages/readme-3d), our open-source toolkit for putting 3D models in any GitHub README, issue, or discussion.
...
<sub>Want your own? `npx readme-3d your-model.glb` converts any GLB into a paste-ready markdown block. [How it works →](https://github.com/nirholas/three.ws/blob/main/packages/readme-3d)</sub>
...
The **3D Studio MCP server** at `https://three.ws/api/mcp-3d` exposes the full pipeline as 15 tools — `text_to_3d`, `image_to_3d`, `auto_rig_model`, `apply_animation`, `stylize_model`, `retexture_model`, `segment_model`, and more — so an AI assistant can generate, rig, and animate a model mid-conversation and render it as an inline interactive artifact. See [docs/mcp-3d-studio.md](https://github.com/nirholas/three.ws/blob/main/docs/mcp-3d-studio.md).

### Pay-per-call for autonomous agents (x402)

`POST /api/x402/forge` is the monetized twin: agents pay per generation in USDC on Base or Solana — no API key, no account. Draft $0.05, standard $0.15, high $0.50; polling is free; retried payments are idempotent and never double-charge. See [docs/api/forge-x402.md](https://github.com/nirholas/three.ws/blob/main/docs/api/forge-x402.md).

### Learn more

- [Tutorial: Turn a Text Prompt into a 3D Model](https://github.com/nirholas/three.ws/blob/main/docs/tutorials/text-to-3d.md) — first model in about a minute
- [Tutorial: Turn Photos into a 3D Model](https://github.com/nirholas/three.ws/blob/main/docs/tutorials/image-to-3d.md) — reconstruct a real object from 1–4 photos
- [3D Studio MCP server](https://github.com/nirholas/three.ws/blob/main/docs/mcp-3d-studio.md) — generate from inside Claude or Cursor
- [Paid generation API (x402)](https://github.com/nirholas/three.ws/blob/main/docs/api/forge-x402.md) — autonomous agent-to-agent generation

---

## Platform Pages

A map of every user-facing route. [`STRUCTURE.md`](https://github.com/nirholas/three.ws/blob/main/STRUCTURE.md) maps each product surface to the directory that implements it, and [`data/pages.json`](https://github.com/nirholas/three.ws/blob/main/data/pages.json) is the registry every public route is generated from (sitemap, `llms.txt`, `features.json`, changelog).
...
three.ws is an **IBM Business Partner**, and the agent runtime runs on **IBM Granite** foundation models served through **IBM watsonx.ai**. One IBM Cloud API key + project unlocks the whole suite; every call is real inference (no mock path — endpoints return `503` when unconfigured). Full docs: **[docs/ibm.md](https://github.com/nirholas/three.ws/blob/main/docs/ibm.md)**. Live showcase: **[three.ws/ibm/galaxy](https://three.ws/ibm/galaxy)**.
...
| ---------------------------------------- | ----------------------------------------------- |
| ![Viewer](https://github.com/nirholas/three.ws/blob/main/public/screenshots/viewer.png) | ![Widget Studio](https://github.com/nirholas/three.ws/blob/main/public/screenshots/studio.png) |

| Agent Discovery                              | Avatar Creation                          |
| -------------------------------------------- | ---------------------------------------- |
| ![Discover](https://github.com/nirholas/three.ws/blob/main/public/screenshots/discover.png) | ![Create](https://github.com/nirholas/three.ws/blob/main/public/screenshots/create.png) |
...
| Spec                                                         | What it covers                                                                                                   |
| ------------------------------------------------------------ | ---------------------------------------------------------------------------------------------------------------- |
| [specs/AGENT_MANIFEST.md](https://github.com/nirholas/three.ws/blob/main/specs/AGENT_MANIFEST.md)           | Agent manifest JSON schema — body, brain, voice, memory, skills, signing. The contract every `<agent-3d>` reads. |
| [specs/3D_AGENT_CARD.md](https://github.com/nirholas/three.ws/blob/main/specs/3D_AGENT_CARD.md)             | The on-chain passport card layout — fields, signing, and rendering rules.                                        |
| [specs/SKILL_SPEC.md](https://github.com/nirholas/three.ws/blob/main/specs/SKILL_SPEC.md)                   | Skill bundle layout (`SKILL.md`, `tools.json`, `handlers.js`), trust modes, and distribution.                    |
| [specs/PERMISSIONS_SPEC.md](https://github.com/nirholas/three.ws/blob/main/specs/PERMISSIONS_SPEC.md)       | EIP-7710 delegated permissions model — capability scopes, redemption, revocation.                                |
| [specs/MEMORY_SPEC.md](https://github.com/nirholas/three.ws/blob/main/specs/MEMORY_SPEC.md)                 | Memory file format, types, salience model, and storage modes.                                                    |
| [specs/STAGE_SPEC.md](https://github.com/nirholas/three.ws/blob/main/specs/STAGE_SPEC.md)                   | Scene/stage configuration: camera presets, lighting, environment maps, hotspots.                                 |
| [specs/EDITOR_SPEC.md](https://github.com/nirholas/three.ws/blob/main/specs/EDITOR_SPEC.md)                 | Widget Studio + Embed Editor configuration surface and persistence shape.                                        |
| [specs/EMBED_SPEC.md](https://github.com/nirholas/three.ws/blob/main/specs/EMBED_SPEC.md)                   | The `<agent-3d>` element and chromeless iframe — attributes, JS API, and lifecycle.                              |
| [specs/EMBED_HOST_PROTOCOL.md](https://github.com/nirholas/three.ws/blob/main/specs/EMBED_HOST_PROTOCOL.md) | `postMessage` wire protocol between the iframe and its host page (origin lock, message kinds, RTT).              |
| [specs/CLAUDE_ARTIFACT.md](https://github.com/nirholas/three.ws/blob/main/specs/CLAUDE_ARTIFACT.md)         | Claude Artifact viewer integration — snippet loading and sandbox boundaries.                                     |
| [specs/ENS_AGENT_CLAIM.md](https://github.com/nirholas/three.ws/blob/main/specs/ENS_AGENT_CLAIM.md)         | ENS-based agent claim flow for verifiable owner ↔ agent binding.                                                |
...
| -------------------------------------------------------------- | -------------------------------------------------------------------- | ------- |
| [Turn a Text Prompt into a 3D Model](https://github.com/nirholas/three.ws/blob/main/docs/tutorials/text-to-3d.md) | A real, textured, downloadable 3D model from a one-line description | ~5 min  |
| [Turn Photos into a 3D Model](https://github.com/nirholas/three.ws/blob/main/docs/tutorials/image-to-3d.md)   | A GLB reconstructed from 1–4 photos of a real object                 | ~10 min |
| [Build Your First Agent](https://github.com/nirholas/three.ws/blob/main/docs/tutorials/first-agent.md)        | A talking 3D character on a shareable page, from zero                | ~20 min |
| [Embed on Your Website](https://github.com/nirholas/three.ws/blob/main/docs/tutorials/embed-on-website.md)    | Add an agent to any page — plain HTML, React, Webflow, WordPress     | ~15 min |
| [Write a Custom Skill](https://github.com/nirholas/three.ws/blob/main/docs/tutorials/custom-skill.md)         | A new tool the agent can call (e.g., fetch live weather data)        | ~30 min |
| [Register On-Chain](https://github.com/nirholas/three.ws/blob/main/docs/tutorials/register-onchain.md)        | Mint your agent onchain — ERC-8004 on EVM or Metaplex Core on Solana | ~20 min |
| [Build a Personal AI Site](https://github.com/nirholas/three.ws/blob/main/docs/tutorials/personal-ai-site.md) | A full personal site with an embedded AI version of yourself         | ~45 min |
...
Only `load` is required; `persist` makes it durable, `recall` makes search semantic (it falls back to substring matching if it throws). To swap *only* the ranker while keeping built-in storage, point `manifest.json → memory.retriever` at a skill instead. Full reference: [specs/MEMORY_SPEC.md → Custom backends](https://github.com/nirholas/three.ws/blob/main/specs/MEMORY_SPEC.md).
...
**Origin contract.** The SDK derives the iframe's origin from `iframe.src` and refuses to start if it can't (no wildcard targets, ever). The iframe locks onto the parent's origin from the first authenticated message it sees and ignores any later messages from a different origin. See [specs/EMBED_SPEC.md](https://github.com/nirholas/three.ws/blob/main/specs/EMBED_SPEC.md) §"Bridge origin model" for the full rules.
...
The **Embed editor** at `/embed` ([src/editor/embed-editor.js](https://github.com/nirholas/three.ws/blob/main/src/editor/embed-editor.js)) is a WYSIWYG configurator for embedding a three.ws avatar or agent on any website. Pick an avatar from the gallery picker, choose an embed mode, tune the environment and size, and copy a ready-to-paste snippet.
...
| Module         | Path                                           | Role                                        |
| -------------- | ---------------------------------------------- | ------------------------------------------- |
| Mannequin      | [src/pose-rig.js](https://github.com/nirholas/three.ws/blob/main/src/pose-rig.js)             | Articulated rig with named joints + IK      |
| Preset library | [src/pose-presets.js](https://github.com/nirholas/three.ws/blob/main/src/pose-presets.js)     | Standing, sitting, action, idle, expressive |
| Studio shell   | [src/pose-studio.js](https://github.com/nirholas/three.ws/blob/main/src/pose-studio.js)       | Scene, controls, export, props, lighting    |
...
The helper handles the 402 challenge, Permit2 sibling, receipt write-back, idempotency-token enforcement, and CSRF/SSRF guards. See [api/\_lib/x402-paid-endpoint.js](https://github.com/nirholas/three.ws/blob/main/api/_lib/x402-paid-endpoint.js).

### Wire checks

- Wire-level CORS, CDP, and Permit2 sibling checks: `tests/e2e/`
- Offer receipts schema + buyer fetch: [api/\_lib/x402-buyer-fetch.js](https://github.com/nirholas/three.ws/blob/main/api/_lib/x402-buyer-fetch.js)
...
4. The lip-sync driver ([`src/voice/lipsync-driver.js`](https://github.com/nirholas/three.ws/blob/main/src/voice/lipsync-driver.js)) samples the analyser every animation frame, extracts amplitude and spectral centroid, and maps them to ARKit-52 blendshape weights.
5. Weights are applied directly to the loaded GLB's morph targets via [`src/voice/avatar-morph-target.js`](https://github.com/nirholas/three.ws/blob/main/src/voice/avatar-morph-target.js) — no scene re-render required.
...
The driver is source-agnostic: it accepts any `AudioBuffer`, so it works identically with ElevenLabs, browser TTS, or a pre-recorded clip. The canonical ARKit-52 blendshape table lives in [`src/voice/arkit-blendshapes.js`](https://github.com/nirholas/three.ws/blob/main/src/voice/arkit-blendshapes.js); per-rig binding (mapping standard names to the specific morph targets in a loaded GLB) is handled by [`src/voice/avatar-morph-target.js`](https://github.com/nirholas/three.ws/blob/main/src/voice/avatar-morph-target.js).
...
| Module        | Path                                             | Role                                                                                                                 |
| ------------- | ------------------------------------------------ | -------------------------------------------------------------------------------------------------------------------- |
| Capture UX    | [src/selfie-capture.js](https://github.com/nirholas/three.ws/blob/main/src/selfie-capture.js)   | Mobile-first 3-shot capture with real-time quality gates (lighting, framing, blur)                                   |
| Pipeline      | [src/selfie-pipeline.js](https://github.com/nirholas/three.ws/blob/main/src/selfie-pipeline.js) | Multi-view fit → FLAME / 3DMM face → base body mesh → rigged GLB                                                     |
...
| Surface             | Path                                                                 | Purpose                                                          |
| ------------------- | -------------------------------------------------------------------- | ---------------------------------------------------------------- |
| Persona extraction  | [api/persona/extract.js](https://github.com/nirholas/three.ws/blob/main/api/persona/extract.js)                     | Short onboarding interview → tone, vocabulary, interests profile |
| Persona preview     | [api/persona/preview.js](https://github.com/nirholas/three.ws/blob/main/api/persona/preview.js)                     | Try the extracted persona against test prompts before saving     |
| Persona keys        | `scripts/generate-persona-key.mjs`                                   | Per-agent signing key + persona SSO setup                        |
| Voice clone modal   | [src/voice/voice-clone-modal.js](https://github.com/nirholas/three.ws/blob/main/src/voice/voice-clone-modal.js)     | 3–10s recording → ElevenLabs custom voice bound to the agent     |
| Talk controller     | [src/voice/talk-controller.js](https://github.com/nirholas/three.ws/blob/main/src/voice/talk-controller.js)         | Push-to-talk and continuous talk modes                           |
| ARKit blendshapes   | [src/voice/arkit-blendshapes.js](https://github.com/nirholas/three.ws/blob/main/src/voice/arkit-blendshapes.js)     | Standard ARKit-52 morph table                                    |
| Lip-sync driver     | [src/voice/lipsync-driver.js](https://github.com/nirholas/three.ws/blob/main/src/voice/lipsync-driver.js)           | Web Audio analyser → blendshape weights per frame                |
| Avatar morph target | [src/voice/avatar-morph-target.js](https://github.com/nirholas/three.ws/blob/main/src/voice/avatar-morph-target.js) | Per-rig binding of ARKit blendshapes to the loaded GLB           |
| Avatar snapshot     | [src/voice/avatar-snapshot.js](https://github.com/nirholas/three.ws/blob/main/src/voice/avatar-snapshot.js)         | Render-time pose capture for thumbnails and OG cards             |
| Persona docs        | [docs/persona-hub.md](https://github.com/nirholas/three.ws/blob/main/docs/persona-hub.md)                           | Full design + onboarding flow                                    |

Memory seed extensions (X, GitHub, Farcaster) feed the agent's memory store at creation time with explicit user consent — see [docs/persona-hub.md](https://github.com/nirholas/three.ws/blob/main/docs/persona-hub.md).
...
| Newsletter     | [api/newsletter-subscribe.js](https://github.com/nirholas/three.ws/blob/main/api/newsletter-subscribe.js) | Resend-backed double-opt-in newsletter              |
...
See [`mcp-server/README.md`](https://github.com/nirholas/three.ws/blob/main/mcp-server/README.md) for full environment variable reference and programmatic client usage.
...
The platform ships with a set of built-in agent skills, packaged in `src/agent-skills-*.js` and registered via [`public/skills-index.json`](https://github.com/nirholas/three.ws/blob/main/public/skills-index.json).
...
Third-party skills are distributed over IPFS / Arweave / HTTP. See [docs/skills.md](https://github.com/nirholas/three.ws/blob/main/docs/skills.md) for the full skill manifest spec and authoring guide.
...
| Module        | Path                                                             | Role                                                                  |
| ------------- | ---------------------------------------------------------------- | --------------------------------------------------------------------- |
| Manager       | [src/animation-manager.js](https://github.com/nirholas/three.ws/blob/main/src/animation-manager.js)             | Load, blend, and crossfade clips per slot (idle, gesture, locomotion) |
| State machine | [src/animation-state-machine.js](https://github.com/nirholas/three.ws/blob/main/src/animation-state-machine.js) | Drives slot transitions from protocol events                          |
| Idle library  | [src/idle-animation.js](https://github.com/nirholas/three.ws/blob/main/src/idle-animation.js)                   | Subtle breath / weight-shift loops that play under everything else    |
...
- Hats, glasses, props attached to named bone slots via [src/agent-accessories.js](https://github.com/nirholas/three.ws/blob/main/src/agent-accessories.js)
...
Supported `provider` values: `anthropic` · `groq` · `openrouter` · `null` (offline). Adding a new provider means implementing the two-method interface (`chat()` and `stream()`) in [`src/runtime/providers.js`](https://github.com/nirholas/three.ws/blob/main/src/runtime/providers.js) — no other files need to change.
...
The gating check runs server-side in [`api/chat.js`](https://github.com/nirholas/three.ws/blob/main/api/chat.js) against the agent's subscription record in Postgres. It cannot be bypassed from the client — the model selection and context limit are applied at the API layer before the request reaches the LLM provider.
...
_Avatars_ ([`api/_mcp/tools/avatars.js`](https://github.com/nirholas/three.ws/blob/main/api/_mcp/tools/avatars.js))
...
_Models_ ([`api/_mcp/tools/models.js`](https://github.com/nirholas/three.ws/blob/main/api/_mcp/tools/models.js))
...
_Solana_ ([`api/_mcp/tools/solana.js`](https://github.com/nirholas/three.ws/blob/main/api/_mcp/tools/solana.js)) — all public, no auth required
...
_Pump.fun_ ([`api/_mcp/tools/pumpfun.js`](https://github.com/nirholas/three.ws/blob/main/api/_mcp/tools/pumpfun.js))
...
See [`contracts/DEPLOYMENTS.md`](https://github.com/nirholas/three.ws/blob/main/contracts/DEPLOYMENTS.md) for current mainnet and testnet addresses. All three registries are deployed via **CREATE2** against a custom vanity-prefixed factory, so the **same address is used on every supported EVM chain** within an environment class — mainnet contracts have one address, testnet contracts another.
...
| Surface       | Path                                                                                 | Purpose                                                                                                                                                                                                               |
| ------------- | ------------------------------------------------------------------------------------ | --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| MCP client    | [api/\_lib/pumpfun-mcp.js](https://github.com/nirholas/three.ws/blob/main/api/_lib/pumpfun-mcp.js)                                  | Cached JSON-RPC client to upstream `pumpfun-claims-bot`                                                                                                                                                               |
| Read API      | [api/agents/pumpfun.js](https://github.com/nirholas/three.ws/blob/main/api/agents/pumpfun.js)                                       | `GET ?op=claims\|graduations\|token\|creator`, plus `?_handler=feed` for the SSE event stream and `?_handler=metadata` for token metadata. Auth: session or bearer (`mcp`/`profile` scope).                           |
...
| Skills        | [src/agent-skills-pumpfun-watch.js](https://github.com/nirholas/three.ws/blob/main/src/agent-skills-pumpfun-watch.js)               | `recent-claims`, `token-intel`, `watch-start`, `watch-stop`                                                                                                                                                           |
| Widget        | [src/widgets/pumpfun-feed.js](https://github.com/nirholas/three.ws/blob/main/src/widgets/pumpfun-feed.js)                           | Live cards overlay                                                                                                                                                                                                    |
...
The crawler runs on a `*/15 * * * *` schedule (see [vercel.json](https://github.com/nirholas/three.ws/blob/main/vercel.json)) and writes into the `pumpfun_signals` table. Agents subscribed via `watch-start` react to incoming events through the existing protocol bus — no new event types required.

Full design and configuration in [docs/solana-pumpfun.md](https://github.com/nirholas/three.ws/blob/main/docs/solana-pumpfun.md).
...
three.ws follows [Semantic Versioning](https://semver.org). The authoritative version lives in [package.json](https://github.com/nirholas/three.ws/blob/main/package.json); the current release is reflected in the badge at the top of this README.
...
| Solidity contracts in `contracts/`                                           | **Stable per deployment** — see [contracts/DEPLOYMENTS.md](https://github.com/nirholas/three.ws/blob/main/contracts/DEPLOYMENTS.md) for addresses | New chains add rows; existing deployments are immutable              |
...
| Area                         | File                                                                                                                                                                                                                                                                                                                                                                         |
| ---------------------------- | ---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| Agent CRUD                   | [tests/api/agents.test.js](https://github.com/nirholas/three.ws/blob/main/tests/api/agents.test.js)                                                                                                                                                                                                                                                                                                                         |
| Agent memory                 | [tests/api/agent-memory.test.js](https://github.com/nirholas/three.ws/blob/main/tests/api/agent-memory.test.js), [tests/src/agent-memory.test.js](https://github.com/nirholas/three.ws/blob/main/tests/src/agent-memory.test.js)                                                                                                                                                                                                                                           |
| Agent protocol bus           | [tests/agent-protocol.test.js](https://github.com/nirholas/three.ws/blob/main/tests/agent-protocol.test.js), [tests/src/agent-protocol.test.js](https://github.com/nirholas/three.ws/blob/main/tests/src/agent-protocol.test.js)                                                                                                                                                                                                                                           |
| Widget CRUD                  | [tests/api/widgets.test.js](https://github.com/nirholas/three.ws/blob/main/tests/api/widgets.test.js)                                                                                                                                                                                                                                                                                                                       |
| Widget types                 | [tests/src/widget-types.test.js](https://github.com/nirholas/three.ws/blob/main/tests/src/widget-types.test.js)                                                                                                                                                                                                                                                                                                             |
| OAuth flow                   | [tests/api/oauth-authorize.test.js](https://github.com/nirholas/three.ws/blob/main/tests/api/oauth-authorize.test.js), [tests/api/oauth-token.test.js](https://github.com/nirholas/three.ws/blob/main/tests/api/oauth-token.test.js), [tests/api/oauth-introspect.test.js](https://github.com/nirholas/three.ws/blob/main/tests/api/oauth-introspect.test.js)                                                                                                                                                             |
| SIWE / SIWS wallet auth      | [tests/api/siwe.test.js](https://github.com/nirholas/three.ws/blob/main/tests/api/siwe.test.js)                                                                                                                                                                                                                                                                                                                             |
| Email + password auth        | [tests/api/auth-email.test.js](https://github.com/nirholas/three.ws/blob/main/tests/api/auth-email.test.js), [tests/api/auth-helpers.test.js](https://github.com/nirholas/three.ws/blob/main/tests/api/auth-helpers.test.js)                                                                                                                                                                                                                                               |
| API keys                     | [tests/api/api-keys.test.js](https://github.com/nirholas/three.ws/blob/main/tests/api/api-keys.test.js)                                                                                                                                                                                                                                                                                                                     |
| LLM proxy                    | [tests/api/llm-anthropic.test.js](https://github.com/nirholas/three.ws/blob/main/tests/api/llm-anthropic.test.js), [tests/api/chat-proxy-ratelimit.test.js](https://github.com/nirholas/three.ws/blob/main/tests/api/chat-proxy-ratelimit.test.js)                                                                                                                                                                                                                         |
| MCP server                   | [tests/api/mcp.test.js](https://github.com/nirholas/three.ws/blob/main/tests/api/mcp.test.js)                                                                                                                                                                                                                                                                                                                               |
| Schema validation            | [tests/api-validate.test.js](https://github.com/nirholas/three.ws/blob/main/tests/api-validate.test.js)                                                                                                                                                                                                                                                                                                                     |
| Crypto utilities             | [tests/api/crypto.test.js](https://github.com/nirholas/three.ws/blob/main/tests/api/crypto.test.js)                                                                                                                                                                                                                                                                                                                         |
| Embed CORS policy            | [tests/api/embed-policy.test.js](https://github.com/nirholas/three.ws/blob/main/tests/api/embed-policy.test.js)                                                                                                                                                                                                                                                                                                             |
| Embed bridge handshake       | [tests/embed-bridge-origin.test.js](https://github.com/nirholas/three.ws/blob/main/tests/embed-bridge-origin.test.js), [tests/embed-bridge-roundtrip.test.js](https://github.com/nirholas/three.ws/blob/main/tests/embed-bridge-roundtrip.test.js)                                                                                                                                                                                                                         |
| Animation slots / state      | [tests/src/animation-slots.test.js](https://github.com/nirholas/three.ws/blob/main/tests/src/animation-slots.test.js), [tests/animation-state-machine.test.js](https://github.com/nirholas/three.ws/blob/main/tests/animation-state-machine.test.js), [tests/animations.test.js](https://github.com/nirholas/three.ws/blob/main/tests/animations.test.js)                                                                                                                                                                 |
| ARKit-52 morphs / lipsync    | [tests/arkit52.test.js](https://github.com/nirholas/three.ws/blob/main/tests/arkit52.test.js), [tests/arkit-blendshapes.test.js](https://github.com/nirholas/three.ws/blob/main/tests/arkit-blendshapes.test.js), [tests/agent-avatar-lipsync.test.js](https://github.com/nirholas/three.ws/blob/main/tests/agent-avatar-lipsync.test.js), [tests/lipsync-driver.test.js](https://github.com/nirholas/three.ws/blob/main/tests/lipsync-driver.test.js), [tests/src/lip-sync-analyser.test.js](https://github.com/nirholas/three.ws/blob/main/tests/src/lip-sync-analyser.test.js)                                       |
| x402 protocol                | [tests/api/x402.test.js](https://github.com/nirholas/three.ws/blob/main/tests/api/x402.test.js), [tests/api/x402-spec.test.js](https://github.com/nirholas/three.ws/blob/main/tests/api/x402-spec.test.js), [tests/api/x402-paid-endpoint-siwx.test.js](https://github.com/nirholas/three.ws/blob/main/tests/api/x402-paid-endpoint-siwx.test.js), [tests/api/x402-gas-sponsoring.test.js](https://github.com/nirholas/three.ws/blob/main/tests/api/x402-gas-sponsoring.test.js), [tests/api/x402-payment-identifier.test.js](https://github.com/nirholas/three.ws/blob/main/tests/api/x402-payment-identifier.test.js) |
| Persona                      | [tests/api/persona.test.js](https://github.com/nirholas/three.ws/blob/main/tests/api/persona.test.js)                                                                                                                                                                                                                                                                                                                       |
| Pump.fun MCP / skills        | [tests/api/pump-fun-mcp.test.js](https://github.com/nirholas/three.ws/blob/main/tests/api/pump-fun-mcp.test.js), [tests/pump-mcp-tools.test.js](https://github.com/nirholas/three.ws/blob/main/tests/pump-mcp-tools.test.js), [tests/pumpfun-ported-skills.test.js](https://github.com/nirholas/three.ws/blob/main/tests/pumpfun-ported-skills.test.js), [tests/src/pump-fun-skill.test.js](https://github.com/nirholas/three.ws/blob/main/tests/src/pump-fun-skill.test.js)                                                                                             |
| Pump.fun pricing / curves    | [tests/api/pump-curve.test.js](https://github.com/nirholas/three.ws/blob/main/tests/api/pump-curve.test.js), [tests/bonding-curve.test.js](https://github.com/nirholas/three.ws/blob/main/tests/bonding-curve.test.js), [tests/pump-swap-ix.test.js](https://github.com/nirholas/three.ws/blob/main/tests/pump-swap-ix.test.js)                                                                                                                                                                                           |
| Pump.fun signals / live feed | [tests/pumpfun-signals.test.js](https://github.com/nirholas/three.ws/blob/main/tests/pumpfun-signals.test.js), [tests/pumpfun-ws-feed.test.js](https://github.com/nirholas/three.ws/blob/main/tests/pumpfun-ws-feed.test.js), [tests/pump-live-stream.test.js](https://github.com/nirholas/three.ws/blob/main/tests/pump-live-stream.test.js), [tests/carbon-graduations.test.js](https://github.com/nirholas/three.ws/blob/main/tests/carbon-graduations.test.js)                                                                                                       |
| Club tips / payouts          | [tests/api/club-tips.test.js](https://github.com/nirholas/three.ws/blob/main/tests/api/club-tips.test.js), [tests/api/club-tips-stream.test.js](https://github.com/nirholas/three.ws/blob/main/tests/api/club-tips-stream.test.js), [tests/api/club-payouts-cron.test.js](https://github.com/nirholas/three.ws/blob/main/tests/api/club-payouts-cron.test.js), [tests/api/dance-tip.test.js](https://github.com/nirholas/three.ws/blob/main/tests/api/dance-tip.test.js)                                                                                                 |
| Club performance / venue     | [tests/club-audio.test.js](https://github.com/nirholas/three.ws/blob/main/tests/club-audio.test.js), [tests/club-camera.test.js](https://github.com/nirholas/three.ws/blob/main/tests/club-camera.test.js), [tests/club-perf.test.js](https://github.com/nirholas/three.ws/blob/main/tests/club-perf.test.js), [tests/club-venue-load.test.js](https://github.com/nirholas/three.ws/blob/main/tests/club-venue-load.test.js), [tests/club-sequence.test.js](https://github.com/nirholas/three.ws/blob/main/tests/club-sequence.test.js)                                                                                 |
| Avatar bake / snapshot       | [tests/avatar-bake.test.js](https://github.com/nirholas/three.ws/blob/main/tests/avatar-bake.test.js), [tests/avatar-snapshot.test.js](https://github.com/nirholas/three.ws/blob/main/tests/avatar-snapshot.test.js), [tests/api/avatar-og.test.js](https://github.com/nirholas/three.ws/blob/main/tests/api/avatar-og.test.js)                                                                                                                                                                                           |
| glTF canonicalize / extras   | [tests/glb-canonicalize.test.js](https://github.com/nirholas/three.ws/blob/main/tests/glb-canonicalize.test.js), [tests/src/gltf-extras.test.js](https://github.com/nirholas/three.ws/blob/main/tests/src/gltf-extras.test.js), [tests/src/validator.test.js](https://github.com/nirholas/three.ws/blob/main/tests/src/validator.test.js)                                                                                                                                                                                 |
| Vanity (Solana + EVM)        | [tests/vanity-wasm-grinder.test.js](https://github.com/nirholas/three.ws/blob/main/tests/vanity-wasm-grinder.test.js), [tests/src/eth-vanity-derivation.test.js](https://github.com/nirholas/three.ws/blob/main/tests/src/eth-vanity-derivation.test.js), [tests/src/eth-vanity-server-verify.test.js](https://github.com/nirholas/three.ws/blob/main/tests/src/eth-vanity-server-verify.test.js), [tests/src/vanity-validation.test.js](https://github.com/nirholas/three.ws/blob/main/tests/src/vanity-validation.test.js)                                             |
| Build asset paths            | [tests/build-asset-paths.test.js](https://github.com/nirholas/three.ws/blob/main/tests/build-asset-paths.test.js)                                                                                                                                                                                                                                                                                                           |
| Agent monetization           | [tests/agent-monetization.test.js](https://github.com/nirholas/three.ws/blob/main/tests/agent-monetization.test.js)                                                                                                                                                                                                                                                                                                         |
| Billing                      | [tests/billing.test.js](https://github.com/nirholas/three.ws/blob/main/tests/billing.test.js)                                                                                                                                                                                                                                                                                                                               |
| Branding / camera presets    | [tests/branding.test.js](https://github.com/nirholas/three.ws/blob/main/tests/branding.test.js), [tests/camera-presets.test.js](https://github.com/nirholas/three.ws/blob/main/tests/camera-presets.test.js)                                                                                                                                                                                                                                                               |
...
| ------------------------------------------------ | ------------------------------------------------------------------------ |
| [tests/e2e/club.spec.js](https://github.com/nirholas/three.ws/blob/main/tests/e2e/club.spec.js) | `/club` venue + HDRI load and audio session within the cold-start budget |

Run with `npx playwright test` (or `npm run test:e2e`). Configuration in [playwright.config.js](https://github.com/nirholas/three.ws/blob/main/playwright.config.js); results in `test-results/` (gitignored).
...
CREATE2 vanity grinds for the multichain factory and payment contracts are recorded in [contracts/DEPLOYMENTS.md](https://github.com/nirholas/three.ws/blob/main/contracts/DEPLOYMENTS.md).
...
The default is Anthropic Claude (`claude-sonnet-4-6` for production, `claude-haiku-4-5-20251001` for low-cost development). Brain routing is configurable per-agent through the manifest and via the `brain` attribute on `<agent-3d>`. Other providers can be wired in by extending [`src/runtime/providers.js`](https://github.com/nirholas/three.ws/blob/main/src/runtime/providers.js).
...
See [CONTRIBUTING.md](https://github.com/nirholas/three.ws/blob/main/CONTRIBUTING.md) for the full contributor guide. Contributors are expected to follow the [Contributor Covenant Code of Conduct](https://github.com/nirholas/three.ws/blob/main/CODE_OF_CONDUCT.md) in every project space — issues, pull requests, discussions, and any community channel that links to this repository.
...
The current threat model and hardening notes live in [specs/SECURITY.md](https://github.com/nirholas/three.ws/blob/main/specs/SECURITY.md) and [docs/security.md](https://github.com/nirholas/three.ws/blob/main/docs/security.md). The [Security Hardening](#security-hardening) section above summarises the in-tree controls.
...
All rights reserved. See [LICENSE](https://github.com/nirholas/three.ws/blob/main/LICENSE).
