...
The skill lives in [`skills/human-writer-en/`](https://github.com/MrBridgeHQ/human-writer-en/blob/main/skills/human-writer-en/): a `SKILL.md` (routing, master checklist, anti-patterns), a `references/` library (stylistic, statistical, and structural tells, humanization techniques, and per-content-type adapters), and `scripts/` (`rules.yaml` plus the `analyze.py` 0-100 scorer and its tests).
