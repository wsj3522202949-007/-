...
- **语言**：Python
- **License**：None
- **Topics**：ai-agents, llm, prompt-engineering, style-guide, technical-writing, writing-style
- **GitHub 描述**：21 writing rules for AI coding and writing agents. Drop-in for Claude Code, Codex, Copilot, Cursor, and Aider, so their output reads like a tech pro.
- **本地描述**：21 writing rules for AI coding and writing agents. Drop-in for Claude Code, Codex, Copilot, Cursor, and Aider, so their output reads like a tech pro.
- **拉取时间**：2026-07-23 23:07:46
...
![agent-style hero: 21 rules, then three real v0.3.0 bench before/after pairs (product description, design-doc section, paper related-work section) with rule violations highlighted in the AI drafts and concrete details highlighted in the agent-style-loaded revisions](https://github.com/yzhao062/agent-style/blob/main/docs/hero.png)
...
![Four writing authorities cited by RULE-01..12: Strunk & White (Elements of Style, 1959), Orwell (Politics and the English Language, 1946), Pinker (Sense of Style, 2014), Gopen & Swan (Science of Scientific Writing, 1990), with the rule IDs distilled from each](https://github.com/yzhao062/agent-style/blob/main/docs/sources.png)
...
See [`RULES.md`](https://github.com/yzhao062/agent-style/blob/main/RULES.md) for the full per-rule blocks with BAD/GOOD examples, enforcement tier, and rationale.

## Does It Work?

![agent-style v0.3.0 sanity bench across Claude Opus 4.7, OpenAI GPT-5.4 via Codex CLI, and Gemini 3 Flash: 105 to 58 (-45%), 51 to 28 (-45%), and 79 to 14 (-82%) AI-tell violations across 10 fixed prose tasks, per-rule breakdown, directional-not-statsig caveat](https://github.com/yzhao062/agent-style/blob/main/docs/bench.png)

Sanity bench on 10 fixed prose tasks (2 PR descriptions, 1 design-doc section, 1 commit message, 4 paper sections, 1 product description, 1 NSF-style specific aim), 2 generations per condition, flagship models drafting with `agent-style` loaded at generation time vs not. Three models shown: **Claude Opus 4.7 dropped 45% (105 → 58 violations); OpenAI GPT-5.4 via Codex CLI dropped 45% (51 → 28); Gemini 3 Flash dropped 82% (79 → 14).** The GitHub Copilot CLI runner was tested but excluded from the figure because its `-p` programmatic mode does not load instruction files (+3% noise; see [`CHANGELOG.md`](https://github.com/yzhao062/agent-style/blob/main/CHANGELOG.md) Notes). See [`docs/bench-0.3.0.md`](https://github.com/yzhao062/agent-style/blob/main/docs/bench-0.3.0.md) for the full scorecard across all 4 runners plus per-runner drafts under [`docs/bench-0.3.0-drafts/`](https://github.com/yzhao062/agent-style/tree/main/docs/bench-0.3.0-drafts/), and [`RELEASING.md`](https://github.com/yzhao062/agent-style/blob/main/RELEASING.md) "Bench (Local Only)" for reproduction steps.
...
Amazon Q Developer, JetBrains AI Assistant, Windsurf, Ollama, Replit, OpenCode, Continue.dev, Tabnine, OpenAI Agents SDK skills, and Copilot path-scoped variants beyond the above are planned for v1.1; see the "Planned adapters" section of [`adapter-matrix.md`](https://github.com/yzhao062/agent-style/blob/main/adapter-matrix.md).
...
This repo is read at generation time. For a linter that runs over committed prose in CI, see [ProseLint](https://github.com/amperser/proselint); per-rule check-ID mappings are in [`enforcement/proselint-map.md`](https://github.com/yzhao062/agent-style/blob/main/enforcement/proselint-map.md). Vale users can plug ProseLint via its existing style-pack ecosystem.
...
Planned CLI additions: `agent-style update` (refresh installed adapters to latest), `agent-style override <RULE-ID> disable` (per-rule opt-out), `agent-style clean` (one-command uninstall), `.agent-style/config.toml` (project-level config), RULE-02 / 07 / 09 / 10 structural detectors filled in, and filled adapters for the planned-adapter set above (v1.1). Track progress in [`CHANGELOG.md`](https://github.com/yzhao062/agent-style/blob/main/CHANGELOG.md).
...
See [`SOURCES.md`](https://github.com/yzhao062/agent-style/blob/main/SOURCES.md) for the full bibliography and recommended reading ranges per source.
...
related:
  ...
  - methods/最强写作方法论_全球最强综合版.md
--- |
| `RULES.md`, `SOURCES.md`, `NOTICE.md`, `agents/`, `adapter-matrix.md` | [CC BY 4.0](https://github.com/yzhao062/agent-style/blob/main/LICENSES/CC-BY-4.0.txt) |
| `enforcement/`, `.github/workflows/`, generator scripts | [MIT](https://github.com/yzhao062/agent-style/blob/main/LICENSES/MIT.txt) |

Every source file carries an SPDX-License-Identifier header. See [`NOTICE.md`](https://github.com/yzhao062/agent-style/blob/main/NOTICE.md) for the attribution snippet consumers should retain on reuse.

## Maintenance

Maintained by [Yue Zhao](https://yzhao062.github.io), USC CS faculty and author of [PyOD](https://github.com/yzhao062/pyod). Issues and pull requests welcome; contributions that add a canonical rule must cite a source from the buckets above. See [`CHANGELOG.md`](https://github.com/yzhao062/agent-style/blob/main/CHANGELOG.md) for release history and upcoming work.
