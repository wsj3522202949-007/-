...
**[➡️ Download the portable build from Releases](https://github.com/MiniDraco/ai-cliche-detector/blob/main/../../releases/latest)** → unzip it anywhere → run **`AI Cliche Detection Suite.exe`**. That's it. The local model is **bundled inside**, so even the deep "Binoculars" perplexity signals work out of the box — **no Python, no GPU, no internet** required.
