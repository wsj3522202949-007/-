...
Most "humanizer" tools are a single hidden prompt behind a paywall. `trueform` is different by design: it's free, runs on your own API key (pennies per document) or fully local via [Ollama](https://github.com/fen-ks/trueform/blob/main/$0), and its quality comes from its **architecture**, not a copy-pasteable prompt.
...
For **real rewrites at $0**, install [Ollama](https://github.com/fen-ks/trueform/blob/main/docs/ollama-setup.md) and use `--provider ollama`.
