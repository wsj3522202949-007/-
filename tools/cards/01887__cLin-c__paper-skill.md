...
窄任务也可以直接调用 `literature`、`figures`、`citations`、`journal`、`reporting` 或 `full-verify`。主 [SKILL.md](https://github.com/cLin-c/paper-skill/blob/main/SKILL.md) 仅负责路由，详细规则按需加载，避免一次塞入全部上下文。
...
4. `journal`：按 [T-RO 官方来源快照](https://github.com/cLin-c/paper-skill/blob/main/references/venues/ieee-tro.md) 重新访问官方页面并生成带访问日期的要求表。
...
|---|related:
  ...
  - methods/最强写作方法论_全球最强综合版.md
---|
| [paper_skill_gate.py](https://github.com/cLin-c/paper-skill/blob/main/tools/paper_skill_gate.py) | 身份、来源、主张证据、引用、报告规范、投稿包、未决输入七项门控 |
| [scholarly_verify.py](https://github.com/cLin-c/paper-skill/blob/main/tools/scholarly_verify.py) | 使用 Crossref/OpenAlex 核验 DOI、标题和年份 |
| [journal_policy_verify.py](https://github.com/cLin-c/paper-skill/blob/main/tools/journal_policy_verify.py) | 保存官方政策 URL、时间、证据片段和内容哈希 |
| [revision_trace.py](https://github.com/cLin-c/paper-skill/blob/main/tools/revision_trace.py) | 核对审稿回复中的修改位置和修订文本 |
| [run_full_workflow.py](https://github.com/cLin-c/paper-skill/blob/main/tools/run_full_workflow.py) | 串联投稿门控、引用、政策和修订一致性检查 |
...
覆盖：完整投稿包、身份信息缺失、无证据主张、断裂证据链接、可疑 DOI、伪 `VERIFIED` 引用、未解决作者输入、报告规范缺口。所有用例位于 [benchmarks/gate-cases.json](https://github.com/cLin-c/paper-skill/blob/main/benchmarks/gate-cases.json)，CI 会在 Windows 和 Linux 上重复运行。
...
- [路线图](https://github.com/cLin-c/paper-skill/blob/main/ROADMAP.md)

项目优先接受可复现的真实科研场景、失败案例、匿名化 before/after 和 benchmark 用例，而不是继续堆叠泛化提示词。

## License

MIT。写作工作流的外部灵感与授权信息见 [ATTRIBUTIONS.md](https://github.com/cLin-c/paper-skill/blob/main/ATTRIBUTIONS.md)。
