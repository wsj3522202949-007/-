...
The skill lives in [`skills/human-writer-hi/`](https://github.com/MrBridgeHQ/human-writer-hi/tree/main/skills/human-writer-hi/): a `SKILL.md` (routing, master checklist, anti-patterns), a `references/` library (stylistic, statistical, and structural tells, humanization techniques, and per-content-type adapters), and `scripts/` (`rules.yaml` plus the `analyze.py` 0-100 scorer and its tests).
