...
title: goat-storytelling-agent
summary: 上述两类主题的补充源，按子主题二次筛选
source: https://github.com/heatzhang/goat-storytelling-agent
created: 2026-07-18
updated: 2026-07-18
no: 7292
category: 画龙补充 / 扩容入库 — 补充源
repo: heatzhang/goat-storytelling-agent
stars: 0
url: https://github.com/heatzhang/goat-storytelling-agent
tier: "C"
...
content_hash: a8b595a3c161bdfb
  - methods/QUICK_START.md
---

# heatzhang/goat-storytelling-agent

- **分类**：画龙补充 / 扩容入库 — 补充源
- **链接**：https://github.com/heatzhang/goat-storytelling-agent
- **Stars**：0
- **语言**：Python
- **License**：MIT
- **Topics**：—
- **GitHub 描述**：Agent that writes consistent and interesting long stories for any fiction form
- **本地描述**：goat-storytelling-agent
- **拉取时间**：2026-07-25 19:16:51
...
![Goat Agent](https://github.com/heatzhang/goat-storytelling-agent/blob/main/images/GOAT-story.png)
## Description
...
The whole pipeline consists of an interplay between different story elements. A whole story can be generated from scratch using the general pipeline. Currently, `HF(TGI)`, `Llama.cpp`, `OpenRouter`, and a `human` in-the-loop text generation backend are supported, but can be extended to any engine.
...
To use the OpenRouter backend you will need an API key from [openrouter.ai](https://openrouter.ai/). Set the key in the `OPENROUTER_API_KEY` environment variable (or edit `goat_storytelling_agent/config.py`) and optionally choose a default model via `OPENROUTER_DEFAULT_MODEL`. The agent automatically includes the HTTP metadata headers recommended by the high-star [open-webui/open-webui](https://github.com/open-webui/open-webui) project when calling the OpenRouter API.

When using the `human` backend, you can omit the endpoint and manually relay prompts to any chat interface (ChatGPT, Claude, etc.). The agent will print the prompt in the terminal and wait for you to paste the model's reply, ending the input with a line containing only `<<END>>`:

```python
from goat_storytelling_agent.storytelling_agent import StoryAgent

writer = StoryAgent(backend_uri='', backend='human', form='novel')
novel_scenes = writer.generate_story('treasure hunt in a jungle')
```

### Command-line usage
The repository ships with a small CLI wrapper that can either coordinate a human handoff workflow or call the OpenRouter API directly:

```bash
# Human-in-the-loop mode (default)
python main.py --topic "treasure hunt in a jungle"

# OpenRouter mode.  You can also set the OPENROUTER_API_KEY environment variable instead of passing --openrouter-api-key.
python main.py --backend openrouter --openrouter-api-key YOUR_KEY --topic "treasure hunt in a jungle"
```

Additional options such as `--openrouter-model`, `--openrouter-endpoint`, `--openrouter-http-referer`, and `--openrouter-title` allow you to override the defaults defined in `goat_storytelling_agent/config.py`.
