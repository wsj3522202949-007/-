...
source: https://github.com/srinivasvssj/ai-detection-paraphrases
created: 2026-07-18
updated: 2026-07-18
no: 7554
category: 画龙补充 / 扩容入库 — 补充源
repo: srinivasvssj/ai-detection-paraphrases
stars: 0
url: https://github.com/srinivasvssj/ai-detection-paraphrases
tier: "C"
...
- **分类**：画龙补充 / 扩容入库 — 补充源
- **链接**：https://github.com/srinivasvssj/ai-detection-paraphrases
- **Stars**：0
- **语言**：None
- **License**：Apache-2.0
- **Topics**：—
...
- **本地描述**：ai-detection-paraphrases
- **拉取时间**：2026-07-25 19:25:28
...
* (Oct 2023) We have added a new setting to utilize the ShareGPT corpus in retrieval-based detection experiments. In [`dipper_paraphrases/detect_retrieval.py`](https://github.com/srinivasvssj/ai-detection-paraphrases/blob/main/dipper_paraphrases/detect_retrieval.py), use the flag `--retrieval_corpus sharegpt`.
...
Script: [`dipper_paraphrases/paraphrase_minimal.py`](https://github.com/srinivasvssj/ai-detection-paraphrases/blob/main/dipper_paraphrases/paraphrase_minimal.py)

*DIPPER manual download*

Checkpoint: https://drive.google.com/file/d/1LJJ1P5X2An0kMn8WAAAJBmxBuNS-5GiK/view?usp=sharing  
To run this downloaded model, in [`dipper_paraphrases/paraphrase_minimal.py`](https://github.com/srinivasvssj/ai-detection-paraphrases/blob/main/dipper_paraphrases/paraphrase_minimal.py), uncomment the line `dp = DipperParaphraser(model="...")` and specify your model checkpoint path.

*SIM model*

You could optionally download the SIM model from Wieting et al. 2021 for calculating semantic similarity of the paraphrased outputs. Download the two files in [this link](https://drive.google.com/drive/folders/1rOOYF3ioDD_Nm0sduhD6ZE1xdUQQKqvG?usp=sharing) and place them in [`dipper_paraphrases/sim`](https://github.com/srinivasvssj/ai-detection-paraphrases/blob/main/dipper_paraphrases/sim).
...
Please run the script [`dipper_paraphrases/paraphrase_minimal.py`](https://github.com/srinivasvssj/ai-detection-paraphrases/blob/main/dipper_paraphrases/paraphrase_minimal.py) and compare the outputs with [`sample_outputs.md`](https://github.com/srinivasvssj/ai-detection-paraphrases/blob/main/sample_outputs.md). The greedy decoded outputs should exactly match, while the top_p samples will have some differences from the sample outputs but have higher diversity.
...
This is all documented in our minimal sample script to run DIPPER, [`dipper_paraphrases/paraphrase_minimal.py`](https://github.com/srinivasvssj/ai-detection-paraphrases/blob/main/dipper_paraphrases/paraphrase_minimal.py), and also in footnote 6 in our [paper](https://arxiv.org/pdf/2303.13408.pdf).
...
Use the scripts [`dipper_paraphrases/generate_gpt2.py`](https://github.com/srinivasvssj/ai-detection-paraphrases/blob/main/dipper_paraphrases/generate_gpt3.py), or [`dipper_paraphrases/generate_gpt3.py`](https://github.com/srinivasvssj/ai-detection-paraphrases/blob/main/dipper_paraphrases/generate_gpt3.py), or [`dipper_paraphrases/generate_opt.py`](https://github.com/srinivasvssj/ai-detection-paraphrases/blob/main/dipper_paraphrases/generate_opt.py) as shown below,
...
Use the scripts [`dipper_paraphrases/paraphrase.py`](https://github.com/srinivasvssj/ai-detection-paraphrases/blob/main/dipper_paraphrases/paraphrase.py) as shown below,
...
We recommend reporting true positive rates at a false positive rate of 1% instead of ROC curves, as discussed in the paper. This will be printed by the script. Nevertheless, the full ROC curves will be stored in `roc_plots`, use [`dipper_paraphrases/plot_roc.py`](https://github.com/srinivasvssj/ai-detection-paraphrases/blob/main/dipper_paraphrases/plot_roc.py) to plot them.

Since DetectGPT takes a while to run, it may be helpful to shard the DetectGPT experiments using the parallel scripts of the previous two steps. Use [`dipper_paraphrases/parallel/merge_json.py`](https://github.com/srinivasvssj/ai-detection-paraphrases/blob/main/dipper_paraphrases/parallel/merge_json.py) to merge the cache. Set `--base_model none` to ignore loading the LLM and just rely on cached results. Also, don't forget the `--base_model` flag in DetectGPT runs, see the code for more details.

For the scaled retrieval experiments, please see [`dipper_paraphrases/detect_retrieval_scale_*.py`](https://github.com/srinivasvssj/ai-detection-paraphrases/blob/main/dipper_paraphrases). Please contact me if you want the raw data accompanying this experiment (email me at kalpesh@cs.umass.edu).
