...
- **门禁**：`tools/scripts/validation/run_all.py` 为唯一权威入口（pre-commit 钩子自动执行），
  含章节自检——跨章重复/模板句/硬短章/重度AI味/无钩子会阻断提交；
  逐章逐段清债明细见 `tools/reports/正文质量清债清单.html`（由 `tools/scripts/writing/生成清债清单.py` 生成）
- **口头禅白名单**：章节目录向上查找 `chapter_oral_tic.txt`（每行一条，`#` 注释），
  命中白名单的重复短语豁免检测；仅限人物设定级口头禅，禁止用来放水刷分
